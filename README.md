...
**小六壬是什么？**
小六壬又称「诸葛马前课」「马前课」，是六壬类术数中最简单的一种：只用手掌六个指节位置（六宫）加起课时间（农历月、日、时辰）推课，不用任何工具，几秒就能掐出来。本应用把它做成了手机上的随身工具和教材。

**六宫是哪六个？**
按顺序是：大安（食指根）→ 留连（食指尖）→ 速喜（中指尖）→ 赤口（无名指尖）→ 小吉（无名指根）→ 空亡（中指根），循环往复。起课算法就是取余：月宫 =(月−1)%6，日宫 =(月宫+日−1)%6，时宫 =(日宫+时辰序)%6。

**怎么起课？**
在「起课」页选一个事类（求财 / 失物 / 行人 / 疾病 / 官事 / 出行 / 求谋 / 交易 / 婚姻 / 宅舍）再点「起课」。日期默认取当前时刻并自动换算农历与时辰，也可以手动改月/日/时。想看完整的掐指过程，去「学习」页看 34 步演示；想自己练，用「我来算」。

**要联网吗？会收集我的数据吗？**
不要联网，也没有任何外部请求和账号体系；课例、练习、错题记录只存在本机（localStorage）。APK 和网页版都是这样。

**断语准不准？能拿来做事决定吗？**
断语摘自传统小六壬通行本，属民俗文化内容，**不构成对任何具体事项的预测或决策建议**，涉及疾病、投资、法律请咨询专业人士。另外要说清楚：课式只由起课时间决定，你选的事类只决定"给你看哪一条断语"，不参与任何计算——这是本项目的产品宪法条款。
